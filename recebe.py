...
#Contador de pacotes para controle e debug
pacotes_dados = 0
pacotes_ignorados = 0
debug_pacote_alto = 0
    ...
    while True:
            ...
            if seq_cmd == CMD_START:
                ...
                if part_sub == SUB_TAMANHO and len(nibbles_tamanho) < 6:
                    ...
                    if recebendo_tamanho:
                        ...
                        if DEBUG == 1:
                            print(f"Recebido nibble de tamanho: {nibble} (total: {len(nibbles_tamanho)}/6)")
                ...
                elif part_sub == SUB_EXTENSAO or (len(nibbles_tamanho) == 6 and not recebendo):
                    codigo_ext = nibble
        ...
        #Se for pacote de DADOS (Bit 7 == 0)
            ...
            if not valida_paridade(nibble, parity):
                if DEBUG == 1:
                    print(f"Pacote com erro de paridade descartado (seq ICMP: {seq})")
                pacotes_ignorados += 1
                continue
            ...
            if part_sub == PART_HIGH:
                ...
                debug_pacote_alto += 1
                if DEBUG_CHATO == 1:
                    print(f"Pacote {debug_pacote_alto} nibble alto recebido: {nibble}")

            #Se for o nibble baixo (bits 3 ao 0)
            elif part_sub == PART_LOW:
                debug_pacote_baixo += 1
                if DEBUG_CHATO == 1:
                    print(f"Pacote {debug_pacote_baixo} nibble baixo recebido: {nibble}")
